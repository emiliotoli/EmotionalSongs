...
public class Canzoni {
   ...
   * @throws IOException
   * @author Stefano Farina
   *         ricerca la canzone, per il titolo passato come parametro al metodo,
   *         nel file Canzoni.dati
   */
  // <editor-fold desc="Ricerca Canzone per Titolo">
  public static void cercaBranoMusicaleTitolo(String Titolo, boolean loggato)
    throws IOException {
    ...
    // se trova la canzone visualizza le sue informazioni che si trovano nel file e
    ...
    if (trovato) {
      ...
      System.out.println("\n");
      // menu con: visualizza emozione e se loggato inserisci emozione terza voce menu
      // principale
      do {
        System.out.println(
          "Digita 1: per visualizzare le emozioni associate a questa canzone: "
        );
        System.out.println(
        ...
        switch (varswich) {
          ...
          case 2:
            if (loggato) {
              System.out.println("inizio procedure per inserire le emozioni");
              Emozioni.inserisciEmozioni(supporto[0]);
              System.out.println(
                "registrazione emozione effetuata con successo\n"
              );
            } else {
              System.out.println("Devi ancora effetuare il login:");
            }
            break;
          case 3:
            varswich = 0;
  ...
  public static void cercaBranoMusicaleAutoreAnno(
    String Autore,
    String Anno,
    boolean loggato
  )
    ...
    while ((sup = bufread.readLine()) != null) {
      ...
      AnnoFile = supporto[2].trim().toLowerCase();
      if (
        Autore.equals(AutoreFile.trim().toLowerCase()) &&
        Anno.equals(AnnoFile.trim().toLowerCase())
      ) {
        trovato = true;
        break;
      }
    ...
    if (trovato) {
      ...
      System.out.println("Anno: " + AnnoFile);
      int varswich;
      // menu--> le operazioni che un utente può fare dopo aver trovato la canzone e
      // se è loggato
      do {
        System.out.println(
          "Digita 1:  per visualizzare le emozioni associate a questa canzone: "
        ...
        switch (varswich) {
          ...
          case 2:
            if (loggato) {
              ...
              System.out.println(
                "registrazione Emozioni effettuata con successo \n"
              );
            } else {
              System.out.println("Devi ancora eseguire il login: ");
            }
            break;
          case 3:
            varswich = 0;
  ...
  public static boolean controlloCanzoneEsistente(String titolo)
    throws IOException {
    File file = new File("data" + sep + "Canzoni.dati.txt");
    String path = file.getAbsolutePath();
    ...
    while ((str = br.readLine()) != null) {
      supporto = str.split("\\|");
      if (
        (supporto[0].toLowerCase().trim()).equals(titolo.toLowerCase().trim())
      ) {
        br.close();
        return true;
      }
  ...
  /**
   * @param titolo
   * @return vero/falso se trova o meno la canzone, questo risultato servirà in un
   *         altro metodo
   * @throws IOException
   * @author Stefano Farina
   *         ricerca la canzone e mi dice il numero della riga in cui si trova nel
   *         file
   */
  ...
  public static boolean ricercaCanzoni(String titolo) throws IOException {
    ...
    while ((srv = br.readLine()) != null) {
      spl = srv.split("\\|");
      if (
        spl[0].contains(titolo) || spl[0].trim().toLowerCase().equals(titolo)
      ) {
        System.out.println(spl[0]);
        System.out.println(spl[1]);
        System.out.println(spl[2]);
        System.out.println("Il codice della canzone è: " + linea);
        trovato = true;
      }
      linea++;
    }
