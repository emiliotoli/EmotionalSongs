...
public class Emozioni {
  ...
  private static final String[] Emozioni = {
    "Amazement",
    "Solemnity",
    "Tenderness",
    "Nostalgia",
  ...
  public static void inserisciEmozioni(String titolo) throws IOException {
    ...
    do {
      do {
        try {
          ...
          System.out.println(
            "puoi inserire solo dei numeri conpresi tra 1 e 9"
          );
    ...
    // passo 2
    System.out.println(
      "motiva la scelta di questa emozione con un breve frase: "
    );
  ...
  public static void visualizzaEmozioni(String tit) throws IOException {
    ...
    double count_Amazement = 0, count_Solemnity = 0, count_Tenderness =
      0, count_Nostalgia = 0, count_Calmness = 0, count_Power = 0, count_Joy =
      0, count_Tension = 0, count_Sadness = 0;
    ...
    while ((sup = bufread.readLine()) != null) {
      ...
      if (
        tit.trim().toLowerCase().equals(supportoTitolo.trim().toLowerCase())
      ) {
        countGenerale++;
    ...
    System.out.println(
      "la percentuale delle emozioni di questa canzone sono: " + "\n"
    );

    String formattedDouble = decimalFormat.format(
      (count_Amazement / countGenerale) * 100
    );
    System.out.println("Amazement = " + formattedDouble + "%");

    formattedDouble =
      decimalFormat.format((count_Solemnity / countGenerale) * 100);
    System.out.println("Solemnity = " + formattedDouble + "%");

    formattedDouble =
      decimalFormat.format((count_Tenderness / countGenerale) * 100);
    System.out.println("Tenderness = " + formattedDouble + "%");

    formattedDouble =
      decimalFormat.format((count_Nostalgia / countGenerale) * 100);
    System.out.println("Nostalgia = " + formattedDouble + "%");

    formattedDouble =
      decimalFormat.format((count_Calmness / countGenerale) * 100);
    ...
    formattedDouble =
      decimalFormat.format((count_Tension / countGenerale) * 100);
    System.out.println("Tension = " + formattedDouble + "%");

    formattedDouble =
      decimalFormat.format((count_Sadness / countGenerale) * 100);
