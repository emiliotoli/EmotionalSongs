...
public class Playlist {
      ...
      do {
        ...
        switch (aggiuntaCanzoni) {
          case "si":
            ...
            if (tmp > 0) str.append("|" + insertByLine(tmp));
            break;
          case "no":
            uscitaciclo = true;
            break;
  ...
  public static boolean controlloPlaylistEsistente(
    ...
    while ((sup = bufread.readLine()) != null) {
      ...
      if (
        IdUtente.equals(supporto[0].trim().toLowerCase()) &&
    ...
    if (!controlloPlaylistEsistente(uid, playlist)) {
      ...
      do {
        ...
        if (scelta.trim().toLowerCase().equals("si")) {
          ...
          if (
            uscitaCicloInterno.trim().toLowerCase().equals("si")
          ) uscitaciclo = false; else uscitaciclo = true;
      ...
      while ((sup = br.readLine()) != null) {
        ...
        if (spl[0].equals(uid) && spl[1].equals(nomeplaylist)) { // se nome utente e nome playlist coincidono
          ...
          for (int i = 2; i < spl.length; i++) {
            if (!(spl[i].trim()).equals(titolo.trim())) stb.append(
              "|" + spl[i]
            );
            if (i >= spl.length - 1) {
              if (j < TOT_PLAYLISTS) stb.append("\n");
            }
          }
        } else {
          if (j > TOT_PLAYLISTS) stb.append(sup); else {
            stb.append(sup);
            stb.append("\n");
          }
        }
  ...
  public static void visualizzaPlaylistUtente(String idUtente)
    throws IOException { // da vedere il senso, se si
    ...
    boolean controlloPlaylist = false;
    ...
    while ((sup = bufread.readLine()) != null) {
      ...
      if (
        ...
        controlloPlaylist = true;
      }
    }
    if (controlloPlaylist == false) {
  ...
  public static boolean cancellaPlaylist(String uid, String nomeplaylist)
    ...
    while ((sup = br.readLine()) != null) {
      ...
      spl = sup.split("\\|");
      if (
        !spl[1].toLowerCase().trim().equals(nomeplaylist.toLowerCase().trim())
      ) {
        stb.append(sup);
        if (j < numTotPlaylist()) stb.append("\n");
      }
    }
